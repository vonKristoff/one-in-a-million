(function () {
	...
	var ui = {
	    ...
	    getters: {
	        hasModalView: function hasModalView(state) {
	            return state.modal
	        }
	    },
	    mutations: {
	        toggleModalView: function toggleModalView(state) {
	            state.modal = !state.modal;
	        },
	        closeModalView: function closeModalView(state) {
	            state.modal = false;
	        }
	    },
	    actions: {}
	...
	var weather = {
	    ...
	    getters: {
	        hasLocation: function hasLocation(state) {
	            return state.location != false
	        },
	        hasWeather: function hasWeather(state) {
	            return state.stats != false
	        },
	        validYetHasNoData: function validYetHasNoData(state, getters) {
	    ...
	    actions: {
	        update: function update(ref, payload) {
	            var commit = ref.commit;
	...
	var appNavigation = {
	    ...
	    template: template,
	    computed: {
	        styleActive: function styleActive() {
	...
	var template$1 = "<div :class=\"styleInactive\" class=\"app-results-component\"><div :style=\"styleGradient\" :class=\"styleHasResults\" class=\"content-background\"></div><div class=\"content-foreground\"><transition name=\"scale\" mode=\"out-in\"><div v-show=\"hasLocation\" class=\"weather-card\"><div class=\"block--location\"><p>{{ location.city }}</p><p>{{ location.region }}</p></div><div v-if=\"hasWeather\" class=\"block--report\"> <p>{{ condition.text }}</p><p>{{ condition.temperature }}&#8451; </p></div><div v-else=\"v-else\" class=\"block--report\"><p class=\"small\">No weather data provided</p></div></div></transition></div></div>";

	var styles = {
	    styleGradient: function styleGradient() {
	        return { 
	            background: ("linear-gradient(" + (this.windDirection) + ", " + (this.tempColour) + " 0%, " + (this.windColour) + " 35%, " + (this.timeColour) + " 100%)")
	        }
	    },
	    styleHasResults: function styleHasResults() {
	        return this.hasLocation ? "is-active" : ""
	    },
	    styleInactive: function styleInactive() {
	...
	var appResults = {
	    name: "app-results",
	    template: template$1,
	    computed: Object.assign({}, vuex.mapGetters('ui', ['hasModalView']),
	...
	var appModal = {
	    ...
	    methods: {
	        ...
	        getForecastData: function getForecastData(ref) {
	            var this$1 = this;
	            var type = ref.type;
	            var payload = ref.payload;
	                ...
	                switch(type) {
	                    case 'geo':
	                        getPosition().then(function (position) {
	                            resolve({ type: type, payload: { geo: { lat: position.coords.latitude, lon: position.coords.longitude }}});
	                        });
	                    break
	                    case 'location':
	                        if(this$1.location.length > 2) { resolve({ type: type, payload: { location: this$1.location }}); }
	                        else { reject(); }
	                    break
	                    default:
	                        resolve({ type: type, payload: { random: { lat: randomDegree(85), lon: randomDegree(180) } }});
	                    break
	                }
	            })
	        },
	...
	window.app = new Vue({
	    el: "#root",
	    store: store,
	    components: { appNavigation: appNavigation, appResults: appResults, appModal: appModal }
	});
